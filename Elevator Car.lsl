...
list FLOOR_ROOT_ZS = [ 31.2, 33.2, 35.2, 37.2, 39.2 ];
...
integer requestDoorsClose(){
    ...
    llRegionSay(CHANNEL, "CLOSE|" + ELEVATOR_ID);
...
default{
    ...
    timer(){
        if (gMoving){
            ...
            llSetTimerEvent(0.0);
            sendArrived();
            if (DEBUG) llOwnerSay("TX: OPEN|" + ELEVATOR_ID);
            llRegionSay(CHANNEL, "OPEN|" + ELEVATOR_ID);
            return;
        }
