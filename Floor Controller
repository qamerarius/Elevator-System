...
integer FLOOR_INDEX = 1; // this landing's floor index (0..4)

float COOLDOWN = 0.6;
float lastClick = -99.0;

integer CHILD_LINK    = 3;
...
integer closeDoors(){
    ...
    gIsMoving = TRUE;
    if (SND_CLOSE != "") llPlaySound(SND_CLOSE, 1.0);
    ...
    llRegionSay(CHANNEL, "DOORS_CLOSED|" + ELEVATOR_ID + "|"+(string)FLOOR_INDEX);
    return 0;
}
...
integer openDoors(){
    if (gIsMoving || !gIsClosed) return 0;
    gIsMoving = TRUE;
    if (SND_OPEN != "") llPlaySound(SND_OPEN, 1.0);
    float dt = STEP_TIME / (float)STEP_COUNT;
    integer i;
...
default
{
    ...
    listen(integer ch, string name, key id, string msg){
        ...
        if (cmd == "CLOSE" && (integer)llList2String(L,2) == FLOOR_INDEX) closeDoors();
        else if (cmd == "OPEN" && (integer)llList2String(L,2) == FLOOR_INDEX) openDoors();
    }
}
